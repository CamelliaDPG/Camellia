...
FunctionPtr StokesVGPFormulation::forcingFunction(int spaceDim, double mu, FunctionPtr u_exact, FunctionPtr p_exact) {
...
void StokesVGPFormulation::initializeSolution(MeshTopologyPtr meshTopo, int fieldPolyOrder, int delta_k,
  ...
  if (_spaceDim==2) {
    ...
    FunctionPtr vorticity = Teuchos::rcp( new PreviousSolutionFunction(_solution, u2_dx - u1_dy) );
    ...
    FunctionPtr u1_soln = Teuchos::rcp( new PreviousSolutionFunction(_solution, this->u(1) ) );
    FunctionPtr u2_soln = Teuchos::rcp( new PreviousSolutionFunction(_solution, this->u(2) ) );
...
FunctionPtr StokesVGPFormulation::getTimeFunction() {
  return _t;
}
