#ifndef CAMELLIA_LOCALFILTER_H
#define CAMELLIA_LOCALFILTER_H
...
// ! TODO: edit this documentation: this is a generalization of LocalFilter that allows selection of a subset of the local rows.

//! LocalFilter a class for light-weight extraction of the submatrix corresponding to local rows and columns.

/*! Class LocalFilter enables a light-weight contruction of an
...
namespace Camellia {
  class LocalFilter : public virtual Epetra_RowMatrix {
  ...
  public:
    ...
    /*!
     ...
     \param
     whichLocalRows - (In) which of the local rows to use
     
     \return Integer error code, set to 0 if successful.
     */
    LocalFilter(const Teuchos::RefCountPtr<const Epetra_RowMatrix>& Matrix, std::vector<int> whichLocalRows);
    ...
    // ! returns the LIDs from the matrix that the the local filter includes
    const std::vector<int> & LIDsInOriginalMatrix()
    {
      return _subdomainRows;
    }
    
  private:
    //! Camellia additions:
    std::vector<int> _subdomainRows;
    std::vector<int> _ordinalLookup; // indices are the local map's ordinals; values are the subdomain ordinals.  -1 for ordinals not present in subdomain
