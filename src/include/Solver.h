...
public:
  ...
  int solve() {
    if (!_saveFactorization) {
      ...
      Teuchos::RCP<Amesos2::Solver<Epetra_CrsMatrix,Epetra_MultiVector> > solver = Amesos2::create<Epetra_CrsMatrix,Epetra_MultiVector>(_solverString, A, X, B);
      solver->solve();
    } else {
      Teuchos::RCP<Epetra_CrsMatrix> A = Teuchos::rcp(dynamic_cast<Epetra_CrsMatrix*>(problem().GetMatrix()), false);
      Teuchos::RCP<Epetra_MultiVector> X = Teuchos::rcp(problem().GetLHS(), false);
      Teuchos::RCP<Epetra_MultiVector> B = Teuchos::rcp(problem().GetRHS(), false);
      _savedSolver = Amesos2::create<Epetra_CrsMatrix,Epetra_MultiVector>(_solverString, A, X, B);
      _savedSolver->solve();
    }
    return 0;
  }
