...
PeriodicBC::PeriodicBC(SpatialFilterPtr pointFilter0, SpatialFilterPtr pointFilter1,
                       FunctionPtr<double> transform0to1, FunctionPtr<double> transform1to0) {
...
PeriodicBCPtr PeriodicBC::periodicBC(SpatialFilterPtr pointFilter1, SpatialFilterPtr pointFilter2, FunctionPtr<double> transform1to2, FunctionPtr<double> transform2to1) {
...
Teuchos::RCP<PeriodicBC> PeriodicBC::xIdentification(double x1, double x2) {
  ...
  FunctionPtr<double> x1_to_x2 = Teuchos::rcp( new TransformXFunction(x1,x2) );
  FunctionPtr<double> x2_to_x1 = Teuchos::rcp( new TransformXFunction(x2,x1) );
...
Teuchos::RCP<PeriodicBC> PeriodicBC::yIdentification(double y1, double y2) {
  ...
  FunctionPtr<double> y1_to_y2 = Teuchos::rcp( new TransformYFunction(y1,y2) );
  FunctionPtr<double> y2_to_y1 = Teuchos::rcp( new TransformYFunction(y2,y1) );
...
Teuchos::RCP<PeriodicBC> PeriodicBC::zIdentification(double z1, double z2) {
  ...
  FunctionPtr<double> z1_to_z2 = Teuchos::rcp( new TransformZFunction(z1,z2) );
  FunctionPtr<double> z2_to_z1 = Teuchos::rcp( new TransformZFunction(z2,z1) );
