...
namespace Camellia {
  ...
  void LinearTerm::addVar(double weight, VarPtr var) {
    FunctionPtr weightFn = (weight != 1.0) ? Teuchos::rcp( new ConstantScalarFunction<double>(weight) )
  ...
  void LinearTerm::addVar(vector<double> vector_weight, VarPtr var) { // dots weight vector with vector var, makes a vector out of a scalar var
    FunctionPtr weightFn = Teuchos::rcp( new ConstantVectorFunction<double>(vector_weight) );
    addVar( weightFn, var );
  }
  ...
  FunctionPtr LinearTerm::evaluate(const map< int, FunctionPtr> &varFunctions, bool boundaryPart) {
    ...
    FunctionPtr fxn = Function<double>::null();
    ...
    for (vector< LinearSummand >::iterator lsIt = summands.begin(); lsIt != summands.end(); lsIt++) {
      ...
      FunctionPtr varEvaluation = Function<double>::op(varFunction,var->op());
      {
        // DEBUGGING CODE:
  ...
  void LinearTerm::values(Intrepid::FieldContainer<double> &values, int varID, FunctionPtr fxn, BasisCachePtr basisCache,
      bool applyCubatureWeights, bool naturalBoundaryTermsOnly) {
    int sideIndex = basisCache->getSideIndex();
...
Intrepid::FieldContainer<double> physicalCellNodes = mesh->physicalCellNodes(elemTypePtr);
...
Intrepid::FieldContainer<double> rieszRepresentation(numCells,numTestDofs);
...
void LinearTerm::computeRieszRHS(Teuchos::RCP<Mesh> mesh){
  ...
  for (elemTypeIt = elemTypes.begin(); elemTypeIt != elemTypes.end(); elemTypeIt++) {
    ...
    Intrepid::FieldContainer<double> physicalCellNodes = mesh->physicalCellNodes(elemTypePtr);
    Intrepid::FieldContainer<double> cellSideParities  = mesh->cellSideParities(elemTypePtr);
    ...
    Intrepid::FieldContainer<double> rhs(numCells,numTestDofs);
...
const map<int,double> & LinearTerm::energyNorm(Teuchos::RCP<Mesh> mesh, IPPtr ip) {
  ...
  Intrepid::FieldContainer<int> activeCellIDs(numActiveElements); // initialized to 0
  Intrepid::FieldContainer<double> norms(numActiveElements);
  Intrepid::FieldContainer<int> numCellsForMPINode(numProcs);
  ...
  for (elemTypeIt = elemTypes.begin(); elemTypeIt != elemTypes.end(); elemTypeIt++) {
    ...
    Intrepid::FieldContainer<double> rhs = _rieszRHSForElementType[elemTypePtr.get()];
    Intrepid::FieldContainer<double> rieszReps = _rieszRepresentationForElementType[elemTypePtr.get()];
...
LinearTermPtr operator/(VarPtr v, FunctionPtr scalarFunction) {
  FunctionPtr one = Teuchos::rcp( new ConstantScalarFunction<double>(1.0) );
...
LinearTermPtr operator-(VarPtr v1, VarPtr v2) {
  FunctionPtr minus_one = Teuchos::rcp( new ConstantScalarFunction<double>(-1.0, "-") );
