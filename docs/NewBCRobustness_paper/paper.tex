...
identified as additional new unknowns.  The result is a hybridized DG \textcolor{blue}{TB: This is not true, the hybridized DG method does not hybridize both traces/fluxes}
method with locally supported optimal test functions, with energy
setting
\begin{align*}
u&\in L^2(\Omega)\\
v&\in V=D(A^*) \\
...
\end{align*}
where $n$ is the outward normal vector to an element edge, and ${\rm sgn}(n)$ is 1 if $n$ aligns with a given global orientation, and $-1$ otherwise. \textcolor{red}{I need to find more simple way to explain sgn} \textcolor{blue}{TB: see my DPG Friedrichs paper for the explanation}.
...
\]
on $\Gamma_-$, and continue \textcolar{blue}{TB: from where?} with the
wall boundary condition $u=0$ on $\Gamma_+$.  For our model problem,
...
In comparison, the quasi-optimal norm has yielded fantastic
\textcolor{blue}{we don't use fantastic in technical English, replace
  it with excellent, for example} results for the Helmholtz equation
...
perspective, looking instead for robust test norms which do not induce
boundary layers in the approximation of optimal test functions, and do
so by formulating a general approach to analyzing the behavior of DPG
under a given test norm.
...
The avoidance of boundary layers in the test norm will be described in more detail in the section detailing numerical experiments. We focus first on quantifying the condition on robustness. Such a condition is equivalent to showing the robust boundedness of the energy error from above and below by norms on $U$ 
\[
...
\]
\textcolor{blue}{As a reader, up to this point, I don't understand what robustness means and why the above double inequalities is the condition for robustness?}
where $\lesssim$ denotes a bound by a constant that will not grow as $\epsilon \rightarrow 0$. %The robust bound on the error in $\|\cdot\|_E$ from below by error in $\|\cdot\|_{U,1}$ is crucial to avoiding the instabilities associated with loss of robustness - 
