...
\]
Ensuring that each term $\|\tau\|_{H({\rm div},\Omega_h)}$ and $ \|v\|_{H^1(\Omega_h)}$ is of the same order will allow for smooth solutions (under some assumptions on $\beta$).  
...
Showing the second condition on robustness is equivalent to showing the robust equivalence of the $L^2$ error of the solution in a norm on $U$ (that does not grow as $\epsilon\rightarrow 0$) times the error in the energy norm
\[
\left(\|u\| + \|\sigma\| + \|\widehat{u}\| + \|\widehat{f}_n\|\right) \lesssim  \left\| \left(u,\sigma,\widehat{u},\widehat{f}_n\right)\right \|_E \lesssim \left(\|u\| + \|\sigma\| + \|\widehat{u}\| + \|\widehat{f}_n\|\right)
\]
...
\subsection{Translate equivalence norms on $U$ to norms on $V$}
...
\begin{align*}
\tau_i\cdot n &= 0, \quad x \in \Gamma_-\\
v_i &= 0, \quad x \in \Gamma_+
\end{align*}
...
We have now decomposed any arbitrary test function $\left(\tau,v\right)$ into a single discontinuous portion and two conforming portions.  Showing robust equivalence of $\|\left(v,\tau\right)\|_V$ with another norm $\|\cdot \|_{?}$ on $V$ reduces to showing robust equivalence of each component 
\[
C_1\|\left(\tau,v\right)\|_{?} \lesssim \|\left(v_{0},\tau_{0}\right)\|_V, \|\left(v_{1},\tau_{1}\right)\|_V, \|\left(v_{2},\tau_{2}\right)\|_V \lesssim C_2\|\left(\tau,v\right)\|_{?}
\]
Bounding $\|\left(\tau_0,v_0\right)\|$ requires the use of techniques and decompositions first developed in \cite{analysisDPG} and adapted to convection-diffusion in \cite{DPGrobustness}.  However, due to the conforming nature of $\left(\tau_1,v_1\right)$ and $\left(\tau_2,v_2\right)$, the bound from above of test functions $\|\left(\tau_{1,2},v_{1,2}\right)\| \leq C\|g\|$ is reduced to proving classical error estimates for the adjoint equation
\begin{align*}
\epsilon^{-1}\tau + \grad v &= f\\
\div \tau - \beta\cdot \grad v &=  g,
\end{align*} 
for data $f, g\in L^2({\Omega_h})$ and boundary conditions $\left.\tau\cdot n\right|_{\Gamma_-} = 0$ and $\left.v\right|_{\Gamma_+} = 0$ 
...
We introduce first the bounds derived; the proofs will be given later.  Under specific assumptions on $\beta$, we have the robust bounds
\begin{itemize}
\item For $f=0$ and $v \in H^1(\Omega_h)$, satisfying boundary conditions \eqref{bc_1} and \eqref{bc_2}
\begin{align*}
\|\beta \cdot \grad v \| &\lesssim \| g\| 
\end{align*}
\item For $v \in H^1(\Omega_h)$ and satisfying boundary conditions \eqref{bc_1} and \eqref{bc_2}, and for sufficiently small $\epsilon$
\begin{align*}
...
\end{align*}
\end{itemize}
Together, these estimates will be sufficient to complete the first step and demonstrate bounds on induced test norms for different scalings of norms on $U$.  

\section{Actually proving things}
