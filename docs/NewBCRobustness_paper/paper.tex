...
\begin{align}
\tau_n = - \epsilon \grad v \cdot n &=f\cdot n, \quad x\in \Gamma_- \label{bc_1}\\ 
...
\begin{lemma} 
\label{lemma_1}
Assume \eqref{a_req} and \eqref{adjoint} hold.  Then, for $v$ satisfying \eqref{bc_1}, \eqref{bc_2} and sufficiently small $\epsilon$, 
\[
...
\begin{lemma} 
\label{lemma_2}
Assume $v$ satisfies $\eqref{adjoint}$,\eqref{bc_1}, and \eqref{bc_2}, and $\beta$ satisfies \eqref{a_req} and \eqref{b_req}.  If $\div f = 0$ and $\epsilon$ is sufficiently small, 
\[
...
\end{align*}
where the boundary norms for $v$ and $\tau$ are defined by duality
\begin{align*}
...
\]
We will bound each term by first decomposing $\left(v,\tau\right) = \left(v_0,\tau_0\right) + \left(v_1,\tau_1\right) + \left(v_2,\tau_2\right)$, where $\left(v_1,\tau_1\right)$ satisfies our adjoint equation \eqref{adjoint} with $f = 0, g= \div \tau - \beta\cdot \grad v$, and $\left(v_2,\tau_2\right)$ satisfies \eqref{adjoint} with $g = 0, f= \epsilon^{-1}\tau + \grad v$.  By construction, $\left(v_0,\tau_0\right)$ must satisfy \eqref{adjoint} with $f=g=0$, and with boundary conditions
\begin{align*}
[v_0] &= [v], \quad x\in \Gamma_h \cup \Gamma_+\\
[\tau_0 \cdot n] &= [\tau \cdot n]-[\tau_2 \cdot n], \quad x\in \Gamma_h\setminus \Gamma_+\\
\end{align*}
...
We've shown the robust bound of a norm on $U$ by energy error; for a full equivalence statement, we require a bound on the energy norm by some norm on $U$.  
\begin{lemma}
Bound from above \textcolor{red}{not done yet}
\end{lemma}
\begin{proof}
\textcolor{red}{not done yet}
\end{proof}
