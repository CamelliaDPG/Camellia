...
continuous test functions). Thus, we seek field variables $u\in L^2(K)$ and
...
We can decompose the discrete flux space $\hat F_h$ into equilibriated fluxes
(zero net flux over each element) and some algebraic complement:
\begin{equation}
\hat F_h=\hat F_h^e\oplus\hat F_h^{ne}\,,\quad\hat 
F_h^{e}:=\{\hat t:\langle\hat t,(1_K,\bs0)\rangle=0\quad\forall K\}\,.
\label{eq:decomposition}
\end{equation}
In theory, we could accomplish this by employing for fluxes in $\hat F_h^{e}$
traces of curls, $\nabla\times\phi$, but we don't do this in practice.

\textcolor{red}{
In practice, we solve \eqref{eq:conservativeConfusionSystem} as a fully
coupled system of equations, but for didactic purposes, we can examine it one
piece at a time. If we restrict \eqref{eq:conservativeConfusionSystem}$_1$ to
$\delta\hat t\in\hat F_h^{ne}$, the Lagrange multiplier term drops out and we
are left with a system that we can solve for $u_h$, $\bsigma_h$, $\hat u_h$,
and $\hat t_h^e\in\hat F^e_h$.
}

\textcolor{red}{
Restricting ourselves to $\delta\hat t\in\hat
F_h^{e}$ in \eqref{eq:conservativeConfusionSystem}$_1$, we obtain a system  of
equations that can be solved for $u_h$, $\bsigma_h$, $\hat u$, and $\hat
t\in\hat F_h^e$. The remaining fluxes, $\hat t\in\hat F_h^{ne}$ are determined
from \eqref{eq:conservativeConfusionSystem}$_2$. Finally, testing with $\hat
t\in\hat F_h^{ne}$ in \eqref{eq:conservativeConfusionSystem}$_1$, we obtain a
system of equations that can be solved for Lagrange multipliers $\lambda_K$.
}

\subsection{\textcolor{red}{Proof of Convergence}}
In the following discussion, we do not account for the error in approximating
the optimal test functions because $\dots$ (Need explanation).
...
Restricting ourselves to $\delta\hat t_h\in\hat F_h^e$, and subtracting
...
\in U_h\times\bs S_h\times\hat U_h\times\hat F_h^e\,.
\nonumber
\end{align}
...
{U_h\times\bs S_h\times\hat U_h\times\hat F_h^e}
}
\frac{\snorm{
...
\norm{(u-u_h,\bsigma-\bsigma_h,\hat u-\hat u_h, \hat t-(\hat t_h^e+\hat t_h^{ne}))}\nonumber\\
&\le\norm{(u-w_h,\bsigma-\bs s_h,\hat u-\hat w_h, \hat t-(\hat r_h^e+\hat t_h^{ne}))}\\
&\hspace{3ex}+\norm{(w_h-u_h,\bs s_h-\bsigma_h,\hat w_h-\hat u_h, \hat r_h^e-\hat t_h^e)}\nonumber
\end{align}
where $w_h\in U_h$, $\bs s_h\in\bs S_h$, $\hat w_h\in\hat U_h$, and $\hat
r_h^{e}\in\hat F_h$ are arbitrary. The inf-sup in kernel condition now
implies:
\begin{align}
\label{eq:estimate}
&\norm{(w_h-u_h,\bs s_h-\bsigma_h,\hat w_h-\hat u_h, \hat r_h^e-\hat t_h^e)}\\
...
\frac{\snorm{
b( (w_h-u_h,\bs s_h-\bsigma_h,\hat w_h-\hat u_h,\hat r_h^e-\hat t_h^e),
(\vdeltau,\taudeltau))
}}
{\norm{(\vdeltau,\taudeltau)}}\nonumber\\
&\hspace{8ex}\le\frac{1}{\alpha}
...
\frac{\snorm{
b( (w_h-u_h,\bs s_h-\bsigma_h,\hat w_h-\hat u_h,\hat r_h^e+\hat t_h^{ne}-\hat t),
(\vdeltau,\taudeltau))
}}
{\norm{(\vdeltau,\taudeltau)}}\nonumber\\
&\hspace{8ex}\le\frac{1}{\alpha}
\norm{(w_h-u_h,\bs s_h-\bsigma_h,\hat w_h-\hat u_h,\hat t-(\hat r_h^e+\hat
t_h^{ne}))}
\end{align}
...
% +\langle\hat r_h^e-\hat t_h^e,\vdeltau\rangle
...
minimum energy extension norm for fluxes. Combining with \eqref{eq:triangle},
we get the final stability result:
\begin{equation}
\norm{(u-u_h,\bsigma-\bsigma_h,\hat u-\hat u_h,\hat t-\hat t_h)}
\leq\left(1+\frac{1}{\alpha}\right)
\inf_{\stackrel{
(w_h,\bs s_h,\hat w,\hat r_h^e)\in}
{U_h\times\bs S_h\times\hat U_h\times\hat F_h^e}
}
\norm{(u-w_h,\bsigma-\bs s_h,\hat u-\hat w,\hat t-(\hat r_h^e+\hat t_h^{ne}))}
\,.
\label{eq:stability}
\end{equation}
...
We look at admissible solutions for the homogeneous Laplace's equation over
