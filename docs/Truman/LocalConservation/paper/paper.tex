...
\sup_{v \in V_0} \frac{\vert a(u,v) \vert}{\Vert v \Vert_V }
 \geq \frac{\vert b(\bfu,T \bfu) \vert}{\Vert \bfu \Vert}
= \sup_{(v,\bftau)}
\frac{\vert b((u,\bfsig,\hat{u},\hat{t}), (v,\bftau)) \vert}{\Vert (v,\bftau) \Vert}
\geq \gamma^2 \Vert (u,\bfsig,\hat{u},\hat{t}) \Vert
\end{equation}
...
Recall the main line of analysis in {\color{red}(supposed to be Brezzi?)}\cite{} leading to the construction of robust
test norms. With the robust test norms, we have
\begin{equation}
\begin{array}{ll}
   \LRp{\Vert u - u_h \Vert^2+ \Vert \bfsig - \bfsig_h
   \Vert^2}^{\frac{1}{2}}
...
restricted version. So, can we claim the robustness in the sense of the inequality above
...
\begin{equation}
\Vert v \Vert_{L^2(K)} \leq \Vert (v,\bftau) \Vert_V \, .
