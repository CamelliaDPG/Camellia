...
We note that, for convection-diffusion problems, we are particularly interested in designing a \textit{robust} DPG method.  Specifically, we are interested in designing methods whose behavior does not change as the diffusion parameter $\epsilon$ becomes very small.  Naive Galerkin methods for convection-diffusion tend to suffer from a lack of robustness; specifically, the finite element error is bounded by a constant factor of the best approximation error, but the constant is often proportional to $\epsilon^{-1}$.  Our aim is to design a DPG method with this in mind.  We follow the methodology introduced by Heuer and Demkowicz in \cite{DemkowiczHeuer}: the ultra-weak variational formulation for convection-diffusion can be refactored as
\[
b\LRp{\LRp{u,\sigma,\uh,\fnh},\LRp{\tau,v}} = \sum_{K\in \Oh}\LRs{\LRa{\fnh,v}_{\delta K} + \LRa{\uh,\tau_n}_{\delta K} + \LRp{u,\div \tau - \beta\cdot\grad v}_{L^2(K)} + \LRp{\sigma,\frac{1}{\epsilon} \tau + \grad v}_{L^2(K)}},
\]
modulo application of boundary data.  If we choose specific \textit{conforming} test functions satisfying the adjoint equations
\begin{align*}
\div \tau - \beta \cdot \grad v &= u,\\
\frac{1}{\epsilon} \tau + \grad v &= \sigma,
\end{align*}
then evaluating $b\LRp{\LRp{u,\sigma,\uh,\fnh},\LRp{\tau,v}}$ at these specific test functions returns back $\norm{u}^2 + \norm{\sigma}^2$, the $L^2$ norm of our field variables.  Multiplying and dividing through by the test norm $\norm{v}_V$, we have
\[
\norm{u}_{\L}^2 + \norm{\sigma}_{\L}^2 = b\LRp{\LRp{u,\sigma,\uh,\fnh},\LRp{\tau,v}} = \frac{b\LRp{\LRp{u,\sigma,\uh,\fnh},\LRp{\tau,v}}}{\norm{v}_V}\norm{v}_V \leq \norm{u,\sigma,\uh,\fnh}_E\norm{v}_V,
\]
where 
\[
\norm{u,\sigma,\uh,\fnh}_E = \sup_{v\in V\setminus\LRc{0}}\frac{b\LRp{\LRp{u,\sigma,\uh,\fnh},\LRp{\tau,v}}}{\norm{v}_V}
\]
is the DPG energy norm.  If we can robustly bound the test norm $\norm{v}_V \lesssim \LRp{\norm{u}_{\L}^2+\norm{\sigma}^2_{\L}}^{1/2}$ (i.e. derive a bound from above with a constant independent of $\epsilon$), then we can divide through to get
\[
\LRp{\norm{u}_{\L}^2 + \norm{\sigma}_{\L}^2}^{\frac{1}{2}} \lesssim \norm{u,\sigma,\uh,\fnh}_E.
\]
...
We note that the construction of the test norm $\norm{v}_V$ for a robust DPG method depends on two things: the test norm, as well as the adjoint equation.  In \cite{DemkowiczHeuer}, the standard problem with Dirichlet conditions enforced over the entire boundary was considered; in \cite{ChanHeuerThanhDemkowicz2012}, boundary conditions were chosen for the forward problem such that the induced adjoint problem was regularized and contained no strong boundary layers, allowing for the construction of a stronger test norm on $V$.  We adopt a slight modification of the test norm introduced in \cite{ChanHeuerThanhDemkowicz2012} for numerical experiments here, which is motivated and explained in more detail in Section~\secref{sec:confusionPlate}.  
...
\begin{array}{lll}
u \in V, p \in Q\\
a(u,v) + c(p,v) & = l(v) & \forall v \in V \\
c(q,u) & = g(q) & \forall q \in Q
\end{array}
\right.
\end{equation}
where $V,Q$ are Hilbert spaces, and $a,c,l,q$ denote the appropriate
...
bound for form $c(p,v)$ representing the constraint.
\begin{equation}
\begin{array}{lll}
\vert b((u,\bfsig,\hat{u},\hat{t}),\sum_K \lambda_K (1_K,{\bf 0})) \vert
& = \vert \sum_K \lambda_K \langle \hat{t}, 1_K \rangle_{\ptl K} \vert \\[8pt]
& = \vert \sum_K \lambda_K \langle v_n , 1_K \rangle_{\ptl K} \vert \\[8pt]
& = \vert \sum_K \lambda_K \int_K \text{div} \bfv \: 1_K  \vert \\[8pt]
& \leq \sum_K  \lambda_K \Vert \text{div} \bfv \Vert_{L^2(K)} \mu(K)^{1/2} \\[8pt]
& \leq (\sum_K \mu(K) \lambda_K^2)^{1/2} \: (\sum_K \Vert \text{div} \bfv \Vert_{L^2(K)}^2 )^{1/2} \\[8pt]
& \leq \underbrace{(\sum_K \mu(K) \lambda_K^2)^{1/2}}_{=: \Vert \bflambda \Vert} \Vert \hat{t} \Vert_{H^{-1/2}(\Gamma_h)}
\end{array}
\end{equation}
Above, function $\bfv$ denotes the $H(\text{div},\Omega)$ extension of flux $\hat{t}$
...
\paragraph{Inf Sup Condition} relating spaces $V$ and $Q$ reads as follows,
\begin{equation}
\sup_{v \in V} \frac{\vert c(p,v) \vert}{\Vert v \Vert_V} \geq \beta \Vert p \Vert_Q
\end{equation}
Let
\begin{equation}
R\, : \, L^2(\Omega) \ni q \to \bfv \in \bfH(\text{div},\Omega) \cap \bfH^1(\Omega)
\end{equation}
be
the continuous right inverse of the divergence operator constructed by
Costabel and McIntosh in \cite{CostabelMcIntosh}.
Let $\bfv_h$ denote the classical, lowest order Raviart-Thomas (RT) interpolant of
function
\begin{equation}
\bfv = R (\sum_K \lambda_K 1_K) \: .
\end{equation}
Note that $\text{div} \bfv_h = \text{div} \bfv = \lambda_K$ in element $K$.
...
\begin{equation}
\begin{array}{lll}
\Vert \bfv_h \Vert & \leq \Vert \bfv_h - \bfv \Vert + \Vert \bfv \Vert\\[8pt]
&\leq C h \Vert \bfv \Vert_{H^1} +  \Vert \bfv \Vert \\[8pt]
& \leq C \Vert \text{div} \bfv \Vert = C (\sum_K \mu(K) \lambda_K^2)^{1/2}
\end{array}
\label{eq:stab}
\end{equation}
...
Let $\hat{t}$ be now the trace of $\bfv_h$. We have then,
\begin{equation}
\sup_{\hat{t} \in H^{-1/2}(\Gamma_h)} \frac{\vert  \sum_K \lambda_K \langle \hat{t},1_K \rangle_{\ptl K} \vert}{\Vert \hat{t} \Vert_{H^{-1/2}(\Gamma_h)}}
\geq \frac{\vert \sum_K \lambda_K \int_K \text{div} \bfv_h \: 1_K  \vert}
{\Vert \bfv_h \Vert_{H(\text{div},\Omega)}}
...
space $V$ makes only the discrete inf-sup constant bigger.
...
\begin{equation}
\begin{array}{rl}
V_0  := & \{ v \in V \, : \, c(v,u) = 0 \: \forall q \in Q\} \\[8pt]
 = &\{ (u,\bfsig,\hat{u},\hat{t}) \, : \, \langle \hat{t},1_T \rangle = 0 \, \forall K \}
\end{array}
\end{equation}
In other words, the kernel space contains only the equilibriated fluxes.
With $u \in V_0$, we have then:
\begin{equation}
\sup_{v \in V_0} \frac{\vert a(u,v) \vert}{\Vert v \Vert_V }
 \geq \frac{\vert b(\bfu,T \bfu) \vert}{\Vert T \bfu \Vert}
= \sup_{(v,\bftau)}
\frac{\vert b((u,\bfsig,\hat{u},\hat{t}), (v,\bftau)) \vert}{\Vert (v,\bftau) \Vert}
\geq \gamma \Vert (u,\bfsig,\hat{u},\hat{t}) \Vert
\end{equation}
...
Recall the main line of analysis in \cite{} leading to the construction of robust
test norms. With the robust test norms, we have
\begin{equation}
\begin{array}{ll}
\Vert u - u_h \Vert, \Vert \bfsig - \bfsig_h \Vert
& \lesssim \Vert (u - u_h, \bfsig - \bfsig_h, \hat{u} - \hat{u}_h, \hat{t} - \hat{t}_h \Vert_E \\[8pt]
&= \inf_{(v_h,\bftau_h,\hat{v},\hat{r}_h)}
\Vert (u - v_h, \bfsig - \bftau_h, \hat{u} - \hat{v}_h, \hat{t} - \hat{r}_h \Vert_E
\end{array}
\end{equation}
The last equality follows from the fact that DPG method delivers the best approximation
error in the energy norm (minimizes the residual). This is not longer true for the
conservative version. So, can we claim the robustness in the sense of the inequality above
...
the original norm of solution $u$ with the energy norm, both constant $\gamma$ and continuity
...
\begin{equation}
\Vert \hat{t} \Vert_E^2 = \sum_K \Vert (v_K,\bftau_K) \Vert_V^2
\end{equation}
We need to establish sufficient conditions under which inf-sup and continuity constants for
...
\begin{equation}
\sup_{\hat{t}} \frac{\vert \sum_K \lambda_K \langle \hat{t},1_K \rangle \vert}{\Vert \hat{t} \Vert_E}
\geq  \beta (\sum_K \mu(K) \lambda_K^2)^{1/2}
\end{equation}
...
of the divergence operator constructed by Costabel and Mac Intosh. The only change compared
...
\label{eq:local}
\end{equation}
We need then an upper bound of the energy norm of $\bfv_h$:
$$
(\sum_K \Vert (v,\bftau) \Vert_V^2)^{1/2}
$$
Substituting $v$ for $\delta v$ in~(\ref{eq:local}), we get,
\begin{equation}
\Vert (v,\bftau) \Vert_V^2 = \lambda_K (1_K, v_K)
\end{equation}
If we have a robust stability estimate:
\begin{equation}
(1_K, v_K) \leq C \mu(K)^{1/2} \Vert (v,\bftau) \Vert_K
\end{equation}
(i.e. constant $C$ is independent of $\epsilon$) then
\begin{equation}
\Vert  (v,\bftau) \Vert_V \leq C \mu(K)^{1/2} \vert \lambda_K \vert
\end{equation}
and, eventually, as needed,
\begin{equation}
\sum_K \Vert (v,\bftau) \Vert_V^2  \leq C^2 \sum_K \mu(K) \lambda_K^2
\end{equation}
which leads to the robust estimate of inf-sup constant $\beta$. For example, it is sufficient if
\begin{equation}
\Vert v \Vert_{L^2(K)} \leq \Vert v \Vert_V \, .
...
\begin{equation}
\vert ((v,\bftau),(1_K,{\bf 0}))_V \vert \leq C \mu(K)^{1/2} \, \Vert (v,\bftau) \Vert_V
\label{eq:robust_est2}
\end{equation}
then
\begin{equation}
\vert \sum_K \lambda_K \langle \hat{t}, 1_K \rangle \vert \leq C (\sum_K \mu(K) \lambda_K^2)^{1/2}
\, (\sum_K \Vert (v,\bftau) \Vert_V^2)^{1/2}
= C (\sum_K \mu(K) \lambda_K^2)^{1/2}  \Vert \hat{t} \Vert_E
\end{equation}
as needed.
...
repeat the same experiments with standard and locally conservative DPG methods
...
calculating Stokes flow over a backward facing step. The stress singularity at
the reentrant corner, in particular, seems to destroy local conservation. We
...
of the conservative and nonconservative formulations is nearly identical once
sufficiently resolved. This
...
Standard and locally conservative DPG perform nearly identically for the
