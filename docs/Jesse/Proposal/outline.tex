...
\]
where $L_{\rm adv} = \div \left(\beta u\right)$ is the first order advective operator, and $L_{\rm diff} = \epsilon \Delta u$ is the second-order diffusive operator.  Let us assume $u$ to be a linear combination of piecewise-linear basis functions $\phi_i, i = 0,\ldots,N$.  The SUPG method is then to solve
\[
