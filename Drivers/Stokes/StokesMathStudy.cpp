...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 2) {
    if (rank == 0) {
      cout << "Usage: ScratchPadDriver [H1Order] [math|qopt]\n";
    }
    return 0;
  }
  double beta = 1e-1; // for quasi-optimal IP
  int H1Order = atoi(argv[1]);
  string normChoice = argv[2];
  bool useMathIP;
  if (normChoice == "math") {
    useMathIP = true;
    if (rank == 0) {
      cout << "Using mathematician's norm.\n";
    }
  } else {
    useMathIP = false;
    if (rank == 0) {
//      cout << "argv[2]: " << argv[2] << endl;
      cout << "Using quasi-optimal norm with beta = " << beta << endl;
    }
  }
...
//  int H1Order = 3;
  pToAdd = 2;
  HConvergenceStudy study = HConvergenceStudy(exactSolution, stokesBFMath, zeroRHS, stokesBC,
                                              useMathIP ? mathIP : qoptIP, 
                                              minLogElements, maxLogElements, H1Order, pToAdd);
  ...
  if (rank == 0) {
    ...
    filePathPrefix << "stokesMath/u1_p" << polyOrder;
    
    study.writeToFiles(filePathPrefix.str(),u1->ID(),u1hat->ID());
    filePathPrefix.str("");
    filePathPrefix << "stokesMath/u2_p" << polyOrder;
    study.writeToFiles(filePathPrefix.str(),u2->ID(),u2hat->ID());
    
    filePathPrefix.str("");
    filePathPrefix << "stokesMath/pressure_p" << polyOrder;
