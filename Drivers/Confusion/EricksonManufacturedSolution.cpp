...
double EricksonManufacturedSolution::solutionValue(int trialID,
  ...
  for (int n = 1;n<25;n++){
    ...
    //    double Cn = cos(n*pi/3.0)-cos(2.0*n*pi/3.0); // discontinuous step 
    //    Cn /= (n*pi);
    double Cn = -n*pi*cos(n*pi/2)+sin(n*pi); // discontinuous hat
    Cn /= (n*pi)*(n*pi);
        
    double Xbottom = exp(r2)*r2-exp(r1)*r1;
    double Xtop = r2*exp(r2)*exp(r1*x)-r1*exp(r1)*exp(r2*x); 
    double dXtop = r1*r2*(exp(r2)*exp(r1*x)-exp(r1)*exp(r2*x));    
    
//    double Xtop = (exp(r1*(x-1))-exp(r2*(x-1)));
//    double Xbottom = (exp(-r1)-exp(-r2));
//    double dXtop = (exp(r1*(x-1))*r1-exp(r2*(x-1)*r2));    

    double X = Xtop/Xbottom;
    double dX = dXtop/Xbottom;
    double Y = Cn*sin(n*pi*y);
    double dY = Cn*n*pi*cos(n*pi*y);
...
void EricksonManufacturedSolution::imposeBC(int varID, FieldContainer<double> &physicalPoints,
  ...
  for (int cellIndex=0; cellIndex<numCells; cellIndex++) {
	...
	if ( abs(x) < 1e-12) { // if not the outflow (pts on boundary already)
	  imposeHere(cellIndex,ptIndex) = true;
	}
      } else if (varID==ConfusionBilinearForm::U_HAT) {
	if (abs(y) < 1e-12 || abs(y-1.0) < 1e-12) {
	  dirichletValues(cellIndex,ptIndex) = solutionValue(varID, physicalPoint);
	  imposeHere(cellIndex,ptIndex) = true;
	}
      }
