...
void MeshRefinementTests::checkMultiElementStiffness(Teuchos::RCP<Mesh> mesh) {
  int numElements = mesh->numActiveElements();
  for (int cellIndex = 0; cellIndex < numElements; cellIndex++) {
    checkMultiElementStiffness(mesh, mesh->getActiveElement(cellIndex)->cellID());
  }
}

void MeshRefinementTests::checkMultiElementStiffness(Teuchos::RCP<Mesh> mesh, int cellID) {
  double tol = 1e-14;
  double maxDiff;
...
//    cout << expectedValues;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in element " << cellID <<  " stiffness computation (using Multi-Basis); maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
}

void MeshRefinementTests::checkPatchElementStiffness(Teuchos::RCP<Mesh> mesh) {
  int numElements = mesh->numActiveElements();
  for (int cellIndex = 0; cellIndex < numElements; cellIndex++) {
    checkPatchElementStiffness(mesh, mesh->getActiveElement(cellIndex)->cellID());
  }
}

void MeshRefinementTests::checkPatchElementStiffness(Teuchos::RCP<Mesh> mesh, int cellID) {
  double tol = 1e-14;
  double maxDiff;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in element " << cellID <<  " stiffness computation; maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
}
...
void MeshRefinementTests::preStiffnessExpectedUniform(FieldContainer<double> &preStiff, 
  ...
  int testID  =  *(testOrder->getVarIDs().begin()); // just one
  int trialID = *(trialOrder->getVarIDs().begin()); // just one (the flux)
...
TEST_F(MeshRefinementTests, TestUniformMeshStiffnessMatrices)
{
  double tol = 1e-14;
  double maxDiff;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in uniform mesh B (with usePatchBasis=false) stiffness computation; maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in uniform mesh B (with usePatchBasis=true) stiffness computation; maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in uniform mesh C (with usePatchBasis=false) stiffness computation; maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
  ...
  EXPECT_TRUE(fcsAgree(expectedValues,actualValues,tol,maxDiff))
    << "Failure in uniform mesh C (with usePatchBasis=true) stiffness computation; maxDiff = " << maxDiff << endl
    << "expectedValues:\n" << expectedValues
    << "actualValues:\n" << actualValues;
}
...
// TEST_F(MeshRefinementTests, TestPatchBasisStiffnessMatrices)
// {
//   checkPatchElementStiffness(_multiA);
//   checkPatchElementStiffness(_multiB);
//   checkPatchElementStiffness(_multiC);
// }

TEST_F(MeshRefinementTests, TestMultiBasisSideParities)
{
  ...
  EXPECT_EQ( A0_side_parities(0,1), A3_side_parities(0,1))
    << "Failure: PatchBasisSideParities: child doesn't match parent along broken side.\n";
  EXPECT_EQ(A3_side_parities(0,1), A4_side_parities(0,1))
    << "Failure: PatchBasisSideParities: children don't match each other along parent side.\n";
  EXPECT_EQ(A3_side_parities(0,1), - A1_side_parities(0,3))
    << "Failure: PatchBasisSideParities: children aren't opposite large neighbor cell parity.\n";
}
