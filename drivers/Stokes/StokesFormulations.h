#ifndef STOKES_FORMULATIONS
#define STOKES_FORMULATIONS
...
public:
  VVPStokesFormulation(double mu) {
    if (mu != 1.0) {
      cout << "\n************************************************\n";
    ...
    _bf = Teuchos::rcp( new BF(varFactory) );
    // tau terms:
    _bf->addTerm(omega_hat,tau->cross_normal());
    _bf->addTerm(p_hat,tau->dot_normal()); // (sigma1, tau1)
    _bf->addTerm(-omega,tau->curl());
    ...
    _graphNorm = Teuchos::rcp( new IP );
    _graphNorm->addTerm( v - tau->curl() ); // omega
