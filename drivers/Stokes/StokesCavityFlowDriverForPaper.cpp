...
int main(int argc, char *argv[]) {
  ...
  for (int refIndex=0; refIndex < refCount; refIndex++) {
    ...
    int fluxDofs = mesh->numFluxDofs();
    int totalDofs = mesh->globalDofCount();
    if (rank==0) {
      cout << "Before refinement " << refIndex << ", energy error = " << energyError;
      cout << " (using " << fluxDofs << " trace dofs and " << totalDofs << " total dofs)." << endl;
    }
  ...
  if (rank==0) {
    ...
    cout << fluxDofs << " trace dofs and " << totalDofs << " total dofs)." << endl;
