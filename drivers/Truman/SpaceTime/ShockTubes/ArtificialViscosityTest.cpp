...
int main(int argc, char *argv[]) {
  ...
  meshBoundary(0,0) =  xmin;
  meshBoundary(0,1) =  tmin;
  ...
  MeshPtr mesh = Mesh::buildQuadMesh(meshBoundary, numX, numT,
  ...
  SpatialFilterPtr init = Teuchos::rcp( new ConstantYBoundary(tmin) );
  ...
  for (int refIndex=0; refIndex<=numRefs; refIndex++)
  {
    ...
    while (L2Update > nlTol && iterCount < maxNewtonIterations)
    {
      ...
      map<int,double> normsSquared = rieszResidual->getNormsSquared();
      // FunctionPtr errorNorms = Function::piecewiseConstant(normsSquared);
