...
public:
  ...
  void values(FieldContainer<double> &values, BasisCachePtr basisCache)
  {
    vector<unsigned> cellIDs = basisCache->cellIDs();
    int numPoints = values.dimension(1);
...
{
  map<unsigned, double> _energyErrorForCell;
public:
  EnergyErrorFunction(map<unsigned, double> energyErrorForCell) : Function(0)
  {
...
int main(int argc, char *argv[])
{
  ...
  // problem params
  double Re = args.Input<double>("--Re","Reynolds number",1e2);
  double dt = args.Input<double>("--dt","Timestep",1);
  double Ma = args.Input<double>("--Ma","Mach number",0.3);
  ...
  double time_tol_orig = args.Input<double>("--timeTol", "time step tolerance",1e-5);
  ...
  {
    mu = T_visc/Re + Teuchos::rcp(new TwoDGaussian(1e-2,.01)); // ~Re = 10, with width 2/Re spread
  }
  ...
  bc->addDirichlet(That, wallBoundary, Function::constant(T_wall*Tscale));
  ...
  HDF5Exporter exporter(mesh, "FlatPlate");
  ...
  for (int k = 0; k < numRefs+1; k++)
  {
    ...
    // check each time if we can switch to the graph norm
    double minSideLength = meshInfo.getMinCellSideLength();
    ...
    while(L2_time_residual > time_tol && (i<numTimeSteps))
    {
      ...
      double newtonTol = 1e-5;//min(max(L2_time_residual/100.0,1e-6),1e-4);  // min newton tol 1e-6, max newton tol 1e-3
      ...
      if (rank==0)
      {
        ...
        cout << "at timestep i = " << i << " with dt = " << 1.0/((ScalarParamFunction*)invDt.get())->get_param() << ", and time residual = " << L2_time_residual << endl << endl;
    ...
    map<unsigned,double> errMap = solution->energyError();
    ...
    if (rank == 0)
      cout << "Exporting solution " << k << endl;
    exporter.exportSolution(backgroundFlow, varFactory, k, 2, cellIDToSubdivision(mesh, 4));
    if (k<numRefs)
    {
      ...
      else
      {
        ...
        map<unsigned,double> xErrMap = rieszResidual->computeAlternativeNormSqOnCells(xip,cellIDs);
        map<unsigned,double> yErrMap = rieszResidual->computeAlternativeNormSqOnCells(yip,cellIDs);

        vector<int> xCells,yCells,regCells;
        map<unsigned,double> threshMap;
